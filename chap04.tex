...
\begin{figure}[h]	
    ...
    \caption{SUS general breakdown}
    \label{fig4.2:susbreakdown}
\end{figure}
\section{Unit testing of backend}
We have to perform these tests:
\begin{itemize}
    \item Insert Cup F, SQL ON LAST returns it; N+1 as well,
    \item Retrueve Cup F (the inserted one),
    \item Insert News F, SQL ON LAST returns it; N+1 as well,
    \item Retrieve New F (the inserted one),
    \item Add 10 availabilities, SQL ON LAST 10 returns them, N+10, 
    \item Retrieve 10 availabilities F (check against prev. inserted),
    \item Insert Club F - SQL ON LAST returns it; N+1 as well,
    \item Retrieve Club F (the inserted one),
    \item Update Pairing - Delete/Insert,
    \item Retrieve Pairing -  the new updated.
\end{itemize}    
\subsection*{PHPUnit (via. Composer) - test execution}
\begin{figure}[h]	
    \centering	
    \includegraphics[scale=0.707]{img/phpunit_term.png}
    \caption{SUS general breakdown}
    \label{fig4.3:phpunitterm}
\end{figure}
