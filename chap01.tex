...
\begin{itemize}
  \item performance issues - shouldn't be problem using LAMP stack, 
  \item larger traffic - can be solved via Kubernetes autoscaling \footnote{\url{https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/}},  
  \item simulaneous application edits - can be solved by locking and comparing hashes of states before comitting,
  \item sessions consistency - can be solved (even throughout cluster restart) using Redis\footnote{\url{https://redis.io}}.
\end{itemize}